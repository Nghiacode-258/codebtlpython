...
class StudentCard(QtWidgets.QFrame):
    ...
    def __init__(self, student: dict, color: str, parent=None):
        ...
        role_color = (
            "#C81E1E" if role == "Lớp trưởng"
            else "#597EF7" if role == "Lớp phó"
            else "#6B7280"
        )
...
class AddStudentDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.role_combo.addItems(["Thành viên", "Lớp trưởng", "Lớp phó"])
        form_layout.addRow("Vai trò trong lớp", self.role_combo)
...
class StudentModal(QtWidgets.QDialog):
    def __init__(self, student: dict, color: str, parent=None):
        ...
        container.setGeometry(0, 0, 500, 260)
        ...
        av = AvatarWidget(student.get("name", "SV"), color, size=80, font_size=28)
        left_layout.addWidget(av, alignment=QtCore.Qt.AlignHCenter)

        name_preview = QtWidgets.QLabel(student.get("name", ""))
        name_preview.setFont(QtGui.QFont("Segoe UI", 11, QtGui.QFont.Bold))
        name_preview.setStyleSheet("color: #111827;")
        name_preview.setAlignment(QtCore.Qt.AlignCenter)
        name_preview.setWordWrap(True)
        name_preview.setMaximumWidth(130)
        left_layout.addWidget(name_preview)

        left_w = QtWidgets.QWidget()
        left_w.setLayout(left_layout)
        left_w.setFixedWidth(140)
        ...
        fields = [
            ("Mã sinh viên", student.get("id", "")),
            ("Tên", student.get("name", "")),
            ("Vai trò trong lớp", student.get("role", "Thành viên")),
        ]
        ...
        for label, value in fields:
            ...
            val.setFont(QtGui.QFont("Segoe UI", 10, QtGui.QFont.Bold))
...
class StudentDashboard(QtWidgets.QWidget):
    ...
    def load_students(self):
        try:
            docs = db.collection("administrative").stream()
            result = []
    ...
    def add_student(self):
        ...
        if dlg.exec_():
            ...
            try:
                db.collection("administrative").add(data)
                ...
                QtWidgets.QMessageBox.critical(self, "Lỗi Firestore", f"Không thể thêm sinh viên: {str(e)}")
    ...
    def create_tab_bar(self):
        ...
        btn = QtWidgets.QPushButton("ⓘ Lớp hành chính")
        btn.setProperty("class", "tabButton")
        btn.setCheckable(True)
        btn.setFixedHeight(48)
        btn.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        btn.setChecked(True)
        self.tab_btn_group.addButton(btn, 0)
        layout.addWidget(btn)
    ...
    def create_class_info_card(self):
        ...
        fields = [
            ...
            ("Vai trò trong lớp", "Lớp trưởng", 1, 0),
        ]
...
if __name__ == "__main__":
    ...
    window.setWindowTitle("Quản lý lớp hành chính")
